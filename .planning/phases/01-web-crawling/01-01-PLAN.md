...
must_haves:
  truths:
    - "Crawl worker integrates correctly with all components (fetcher, robots, rate limiter, classifier)"
    - "Pages discovered via sitemap are added to queue and crawled"
    - "External social links (LinkedIn, Twitter, Facebook) are detected and recorded"
    - "Crawl respects max_pages and max_depth configuration"
  artifacts:
...
<task type="auto">
  ...
  <files>backend/tests/fixtures/crawl_fixtures.py</files>
  <action>
Create shared fixtures for crawl integration testing:

1. Create `backend/tests/fixtures/__init__.py` (empty)
