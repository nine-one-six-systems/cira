...
<task type="auto">
  <name>Task 1: Create crawler edge case tests</name>
...
1. **Test class: TestMalformedContent**
   ...
   - `test_handles_extremely_large_page`:
     - Return 10MB of HTML content
     - Assert crawler handles it (possibly with truncation)
     - Assert memory is not exhausted
...
3. **Test class: TestSitemapEdgeCases**
...
4. **Test class: TestRobotsEdgeCases**
...
Use fixtures from Plan 01 where applicable.
  ...
  <verify>
```bash
cd /Users/stephenhollifield/Cira && python3 -m pytest backend/tests/test_crawl_edge_cases.py -v --tb=short 2>&1 | head -80
```
  </verify>
  <done>All edge case tests pass, demonstrating crawler robustness against production error conditions.</done>
</task>

<task type="auto">
  <name>Task 2: Add rate limiter stress tests</name>
  <files>backend/tests/test_crawl_edge_cases.py</files>
  <action>
